...
public class ParenthesesSequenceExt {
    ...
    private static boolean isBalanced(String sequence) {
        ...
        for(int i=0;i<n;i++){
            if(sequence.charAt(i)==LEFT_PAREN)roundBraket++;
            if(sequence.charAt(i)==RIGHT_PAREN)roundBraket--;
            if(sequence.charAt(i)==LEFT_BRACE)figureBraket++;
            if(sequence.charAt(i)==RIGHT_BRACE)figureBraket--;
            if(sequence.charAt(i)==LEFT_BRACKET)squareBraket++;
            if(sequence.charAt(i)==RIGHT_BRACKET)squareBraket--;
