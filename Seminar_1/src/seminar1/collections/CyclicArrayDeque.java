...
public class CyclicArrayDeque<Item> implements IDeque<Item> {
    ...
    public void pushFront(Item item) {
        ...
        /* TODO: implement it */
        if(front==back&&elemCount==0){elementData[front]=item; elemCount++; return;}
        if(front+1>=size){
            if(back<=0)grow();
            else {
                front=0;
            }
        }else if(front+1==back)grow();
        front++;
        elemCount++;
    ...
    public void pushBack(Item item) {
        /* TODO: implement it */
        if(front==back&&elemCount==0){elementData[back]=item; elemCount++; return;}
        if(back-1<0){
            if(front+1>=size)grow();

        }else if(back-1==front)grow();
    ...
    public Item popFront() {
        ...
        if(elemCount==1){elemCount--; return item;}
        if(front-1<0){front=size-1;}
        else{
        front--;
        }
        elemCount--;
        if(elemCount*4<size)shrink();
        return item;
    }
    ...
    public Iterator<Item> iterator() {
        ...
        return new Iterator<Item>() {
            ...
            public Item next() {
                ...
                }else if(back==front)return null;
                else {
                    if(back+1!=size) return elementData[back+1];
