...
public class CyclicArrayQueue<Item> implements IQueue<Item> {
    ...
    public void enqueue(Item item) {
        ...
        if(front<0){
            if(rear==maxSize-1)grow();
            else  front=maxSize-1;

        }else if(front==rear&&elemCount!=0)grow();

        if(front<0)front=maxSize-1;
        if(elemCount==0){front=rear=maxSize-1;}
    ...
    public Item dequeue() {
        ...
        rear--;
        if(rear==-1&&elemCount>1)rear=maxSize-1;
        elemCount--;
        if(elemCount*4<maxSize&&elemCount>1) shrink();
    ...
    public Iterator<Item> iterator() {
        ...
        return new Iterator<Item>() {
            ...
            @Override
            public Item next() {
                if(elemCount<=1)return null;
                else {
                    if(rear==0)return elementData[maxSize-1];
                    else return elementData[rear-1];
                }
